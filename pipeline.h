...
int (*DE_9IM_function)(uint &idA, uint &idB, unordered_map<uint,unsigned long> &offsetMapR, unordered_map<uint,unsigned long> &offsetMapS, ifstream &finR, ifstream &finS);
// for testing
int (*join_DE9IM_pointer)(Polygon *polA, Polygon *polB);
...
// Define a function to write IDs to a file
void writeIDsToFile(uint idA, uint idB, const char* filename) {
    ofstream outputFile(filename, ios_base::app); // Open file in append mode
    if (outputFile.is_open()) {
        outputFile << idA << "," << idB << endl; // Write IDs to file
        ...
        if(COMPRESSION == 0){
                ...
                if(DATA_TYPE == POLYGON_TYPE){
                        ...
                        if(WITHIN){
                                ...
                                DE_9IM_function = &refinement_DE9IM_WithIDs;

                                join_DE9IM_pointer = &joinPolygons_DE9IM;
...
void forwardCandidatePair(uint idA, uint idB){
        ...
        switch ((*DE_9IM_function)(idA, idB, offsetMapR, offsetMapS, finR, finS)) {
                ...
                case 9:
                        ...
                        break;
                /*case 2:
                        writeIDsToFile(idA, idB, "Results/R_inside_S_pairs.txt");
                        RinsideS += 1;
                        break;
                case 3:
                        SinsideR += 1;
                        // Write IDs to file for S inside R
                        writeIDsToFile(idA, idB, "Results/S_inside_R_pairs.txt");
                        break;
                case 4:
                        writeIDsToFile(idA, idB, "Results/R_covered_By_S_pairs.txt");
                        RcoveredByS += 1;
                        break;
                case 5:
        ...
        vector<uint> commonSections = DATA_SPACE.getCommonSectionsIDOfObjects(idA, idB);
                for(auto &secID : commonSections){
                        ...
                        if(result2==3) {
                                ...
                                if((*DE_9IM_function)(idA, idB, offsetMapR, offsetMapS, finR, finS)==8){
                                        // Write IDs to file for S inside R
                                        //writeIDsToFile(idA, idB, "R_contains_S_pairs_DE9IM.txt");
                                        RcontainsS_DE9IM++;
                                        writeIDsToFile(idA, idB, "R_contains_S_pairs_DE9IM.txt");
